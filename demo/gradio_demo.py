...
class VibeVoiceDemo:
    ...
    def generate_podcast_streaming(self, 
        ...
        try:
            ...
            min_yield_interval = 3 # Yield every 5 seconds
            min_chunk_size = sample_rate * 6
...
Or paste text directly and it will auto-assign speakers.""",
        ...
        def generate_audio_file_only(num_speakers, script, speaker_1, speaker_2, speaker_3, speaker_4,
                                     cfg_scale, disable_voice_cloning):
            """
            Dedicated API endpoint that ignores streaming and just returns the final WAV file.
            """
            # Iterate through the generator to completion
            final_audio = None
            iterator = demo_instance.generate_podcast_streaming(
                num_speakers=int(num_speakers),
                script=script,
                speaker_1=speaker_1,
                speaker_2=speaker_2,
            ...
            for streaming_chunk, complete_audio_tuple, log, _ in iterator:
                if complete_audio_tuple is not None:
                    final_audio = complete_audio_tuple
        ...
        # --- ADD THIS FUNCTION FOR STREAMING CHUNKS ---
        def generate_chunks_only(num_speakers, script, speaker_1, speaker_2, speaker_3, speaker_4, cfg_scale,
            ...
            for streaming_audio, _, _, _ in iterator:
                # streaming_audio is a tuple: (sample_rate, numpy_array)
                if streaming_audio is not None:
                    yield streaming_audio


        # --- ADD THIS HIDDEN BUTTON FOR API ACCESS ---
        api_btn = gr.Button(visible=False)
        api_btn.click(
            fn=generate_audio_file_only,
            inputs=[num_speakers, script_input] + speaker_selections + [cfg_scale, disable_voice_cloning],
            outputs=[complete_audio_output],  # ONLY return the final audio file
            api_name="generate_wav"  # The new clean endpoint name
        )

        # --- ADD THIS HIDDEN COMPONENT AND BUTTON ---
        # Note: streaming=False ensures we get WAV files for each chunk, not a playlist
        api_chunk_output = gr.Audio(visible=False, streaming=False)

        chunk_btn = gr.Button(visible=False)
        chunk_btn.click(
            fn=generate_chunks_only,
            inputs=[num_speakers, script_input] + speaker_selections + [cfg_scale, disable_voice_cloning],
            outputs=[api_chunk_output],
            api_name="generate_stream"  # <--- The endpoint name for your script
        )
